...
class ScreenAlphaBeta:
    ...
    def _launch_ai(self):
        ...
        depth     = self._depth
        def worker():
            ai  = AlphaBeta(depth=depth, size=BOARD_SIZE)
            self._ai_result = ai.get_move(grid_copy)
        self._ai_result = None
        self._poll_thread = threading.Thread(target=worker, daemon=True)
        self._poll_thread.start()

    def _apply_ai(self, res):
        if res.move is None: self.state="playing"; self.current=HUMAN; return
        r, c = res.move
        self._turn_no += 1
    ...
    def _draw_panel(self):
        ...
        surf.blit(f["title"].render("ALPHA-BETA", True, C_ACC), (px, 6))
        draw_label(surf, "Human vs Minimax + Alpha-Beta pruning", px, 36, f["small"])
    ...
    def _draw_status(self):
        surf = self.screen; f = self.fonts["header"]
        ...
        surf.blit(t, t.get_rect(center=(cx, BY-38)))
